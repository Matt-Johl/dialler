...
# dev-server`), with the harness's phone-b (202, in docker) as the caller.
#
...
#   make harness-ring-native        # terminal 2: 202 dials 201 (the phone)
set -eu
cd "$(dirname "$0")/.."
...
echo "== phone-b (202, docker) → native server at $HOST → dials 201 (the phone)"
