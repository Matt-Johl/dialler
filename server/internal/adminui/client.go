...
// Created is what adding a device returns: its identity.
type Created struct {
...
// CreateDevice adds a device with the given id (generated by the server
// when empty) for user, with an optional label. No enrolment code is
// minted: the admin sets the device up first and asks for one after.
func (c *Client) CreateDevice(ctx context.Context, deviceID, user, label string) (Created, error) {
	var out Created
	err := c.do(ctx, http.MethodPost, "/v1/admin/devices", map[string]any{"device_id": deviceID, "user": user, "label": label, "code": false}, &out)
	return out, err
}
