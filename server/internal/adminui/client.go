...
// when empty) for user, with an optional label, and mints its first
// enrolment code.
func (c *Client) CreateDevice(ctx context.Context, deviceID, user, label string) (Created, error) {
	var out Created
	err := c.do(ctx, http.MethodPost, "/v1/admin/devices", map[string]string{"device_id": deviceID, "user": user, "label": label}, &out)
	return out, err
}
