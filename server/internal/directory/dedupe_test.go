...
// Re-seeding a directory (harness/provision.sh runs on every harness-up)
// must update the existing contact for a URI, not add another one.
func TestUpsertByURIDoesNotDuplicate(t *testing.T) {
	s, err := Open(filepath.Join(t.TempDir(), "directories"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestUpsertCollapsesExistingDuplicates(t *testing.T) {
	...
	delta, _ := s.Changes("dev-a", sinceCollapse)
	tombstones := 0
	for _, c := range delta {
		if c.Deleted {
			tombstones++
